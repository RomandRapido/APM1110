...
As one can notice, as the size of the experiment becomes larger, the relative frequency for both outcome (heads and tails) approaches 0.50. This goes to show that the probability of both heads and tails is $\textbf{0.50}$.
...
Similar to the graph in the first item (a), both outcome seems to be approaching 0.50 and thus the probability for both red and black is $\textbf{0.50}$.
...
ggplot(c_long_df, aes(x = sample_size, y = rel_freq, color = result)) +
  ...
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red")
```
Similar to the graphs of both numbers above, both outcome seems to be approaching 0.50 and thus the probability for both odd and even numbers is $\textbf{0.50}$.
...
To show the validity of the claim the they are indeed around 0.1667, here is the average of rel_freq:
``` {r}
