...
Before anything else though, let's convert the percentage to their decimal
equivalent. I will also change the column names for readability and
accuracy purposes.
...
To get the overall probability (previously percentage) of relevant images or the probability of relevant images with regards to the whole sample space ( $P(E)$ ), we'll first have to get the percentage of the relevant images given to each sensors.
...
To get the overall percentage of relevant images, we can assert the Law of Total Probability:
...
Show that $E_1$ , $E_2$ , and $E_2$ are pairwise independent but not mutually independent.
...
However, the three pair of events mentioned in the given can only become mutually independent if:
```
...
When tossing a coin, the probability of getting a side (either heads or tails) is $\frac{1}{2}$

Thus, when we toss two coins successively, the probability of all the result will be: 
...
$$
Also, as one can notice, if we exhausts all the possible pairs of all three given events, their intersections will just consists of 1 element.

For $E_1 \cap E_2$ , $E_2 \cap E_3$ , and $E_1 \cap E_3$ alike, its "HH."
Giving us the probability of $\frac{1}{4}$ for all the pairs.
...
But if we look at the possible outcomes, only "HH" can be found among all three events. Thus making $P(E_1 \cap E_2 \cap E_3) = \frac{1}{4}$.

However, $\frac{1}{4} \neq \frac{1}{8}$ .

Making $E_1$ , $E_2$ , and $E_3$ NOT mutually independent.

Therefore, $E_1$ , $E_2$ , and $E_2$ are pairwise independent but not mutually independent.
